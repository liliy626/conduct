...
_FINAL_REQUEST_LAYER_PROTOCOL = (
    "最终请求层级说明：最终发给模型的请求由“用户问题、数据上下文、system prompt / style guard、LLM messages”组成。"
    "用户问题是本轮唯一需要回答的目标；数据上下文是回答数据问题时的唯一事实依据，必须优先于历史消息中的旧结论；"
    "system prompt / style guard 规定安全边界、输出口径和格式要求，必须持续遵守；"
    "LLM messages 是原始对话历史，历史消息只作为对话上下文，不得用历史消息覆盖当前检索结果。"
    "如果历史消息与当前数据上下文冲突，以当前数据上下文为准，并用简短自然语言说明口径。"
)

_GLOBAL_ANSWER_STYLE_WITH_DATA = (
    "回答风格要求：请统一使用更有人情味、贴近校内沟通的汇报口吻。"
    "默认用中文自然语言回答，像面向校长、年级组长或老师的校内简报。"
    "先给出清晰结论，再解释数据反映出的实际情况。"
    "如果发现风险、遗漏或流程问题，请给出简短、可执行的下一步建议。"
    "回答结构优先包含两部分：简短结论 + 数据支撑。"
)

_GLOBAL_ANSWER_STYLE_NO_DATA = (
    "回答风格要求：统一使用中文、自然、温和、专业的校内沟通口吻。"
    "回答尽量先说结论，再做简要说明；避免生硬、模板化。"
)

_GLOBAL_ANSWER_STYLE_JSON = (
    "回答风格要求：用户明确要求结构化结果时，直接输出合法、完整、机器可读的JSON。"
    "JSON之外不要补充任何自然语言。"
)
...
def audience_answer_guard(
    *,
    ...
    if mode == "principal":
        return (
            "受众口径要求（校长）：回答必须采用“结论先行 + 风险分级 + 资源与决策建议”。"
            "先用1段给出总判断；再给出风险分级（高/中/低），每级写清触发依据；"
            "最后给出3条以内资源与决策建议，建议必须可执行。"
        )
    return (
...
def build_class_grade_portrait_note(scope: str = "class") -> str:
    ...
    if mode == "grade":
        return (
            ...
            "3) 异常预警（晨午检+请假的群体风险）；"
            "4) 学风研判（作业执行与文印协同）。"
            "分析要求：识别领头羊班级与需关注班级；给出下周管理难点预测；输出年级管理建议。"
            "输出结构固定为：年级关键指标看板 -> 班级差异矩阵 -> 风险预警 -> 年级管理策略。"
            "只允许依据检索结果里的班级/年级数据，不得补充不存在的班级表现。"
        )
    return (
        "画像任务（班级画像）：你是一位资深班主任导师与教育数据分析专家。"
        "请基于检索结果生成“班级健康度画像”，将离散数据转成多维特征分析。"
        "分析维度：1) 基础情况；2) 晨午检；3) 作业；4) 文印；5) 行规；6) 请假。"
        "分析要求：给出3-5个核心标签；做跨维度关联分析（如作业与文印、行规与请假）；"
        "指出当前最急需解决的1个痛点并给出行动建议。"
        "输出结构固定为：标签展示 -> 现状扫描 -> 深度洞察 -> 行动建议。"
...
def build_student_teacher_portrait_note(scope: str = "student") -> str:
    ...
    if mode == "teacher":
        return (
            "画像任务（教师画像）：你是一位学校人力资源专家和教学质量评估顾问。"
            "请基于检索结果构建教师职业画像，强调建设性改进，不做主观定性。"
            "分析逻辑："
            "1) 能量状态分析（工作安排+执勤+晨午检作为事务性支出，教研活动作为专业成长输入）；"
            "2) 教学风格推断（学生评价与业务轨迹联动）；"
            "3) 责任心建模（执勤/晨午检执行完整性）。"
            "输出结构固定为：教师画像摘要 -> 职业负荷雷达图建议 -> 学生视角还原 -> 管理策略。"
            "约束：结论必须基于检索结果的真实数据，不得编造；风险表达使用“疑似/可能”等客观措辞。"
        )
    return (
        "画像任务（学生画像）：你是一位青少年心理学专家与教育数据分析师。"
        "请基于检索结果构建学生个体画像，识别学习驱动力与潜在风险。"
        "分析逻辑："
        "1) 稳定性分析（请假/离校轨迹与产出表现联动）；"
        "2) 兴趣图谱（活动/作品类型与跨学科倾向）；"
        "3) 合规性与自律（档案、请假、活动等多维交叉）。"
        "输出结构固定为：核心人格标签 -> 行为规律总结 -> 能力象限评估 -> 教育建议。"
        "约束：必须基于检索结果的真实数据，不猜测；风险项使用“疑似/可能”等客观词汇。"
    )
