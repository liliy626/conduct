...
【默认视觉风格】
生成一张现代 BI 数据可视化页面，整体参考 Apple + Notion + Linear + 飞书 Analytics。
采用浅色高级 SaaS BI 风格：白色与浅灰背景、大量留白、圆角卡片、轻阴影、企业级运营分析平台质感。
配色使用低饱和高级色：清爽蓝、薄荷绿、柔和橙、浅紫、珊瑚红、天蓝色；背景可使用 #F7F9FC、#FFFFFF、#EEF2F7；文字使用深蓝黑和蓝灰色。
不要传统深蓝科技大屏，不要 HUD，不要赛博朋克，不要霓虹发光，不要黑色背景，不要复杂装饰纹理，不要高饱和荧光色。

【图表与版式】
画面比例优先 16:9，也可以 4:3；像真实智慧校园 BI 系统中的专业分析模块。
顶部要有清晰中文标题和统计周期；关键指标用 3-4 个 KPI 卡片呈现。
...
【数据约束】
数字绝对不能乱生成。所有数字、日期、百分比、排名、峰值、环比、同比、合计、均值、坐标轴标签和业务结论，必须严格来自用户 prompt 中明确提供的数据。
不要自行推算、补全、改写或美化任何未提供的数字；不要把示例数字、占位数字或常识数字画进图里。
如果用户 prompt 没有提供某个 KPI、环比、百分比、最高峰值、统计周期或分类合计，就不要显示该项；可以改成“未提供数据”或直接省略该卡片/标签。
如果用户 prompt 只提供了少量数据，只画这些已提供数据对应的图形；不要为了填满 31 天、完整坐标轴或底部汇总而虚构日期和数值。
图表中的横轴日期、纵轴刻度、折线点、柱状图高度、气泡标签、底部汇总数字都必须与用户 prompt 中的数据一致。
如果无法保证中文数字完全准确，优先减少图中文字和数字，只保留标题、真实统计周期、已提供的关键数字和简洁图例。
必须优先使用用户 prompt 中提供的真实数据、日期、指标和分类；不要编造与用户数据冲突的数字。
如果用户 prompt 中没有具体数字，可以做概念型 BI 视觉，但必须明确是“示意图”，不要假装为真实统计结果。
不要生成真实人物肖像、学生/教师个人隐私明细或虚构学校 logo。
""".strip()
...
def _image_scene_hint(prompt: str) -> str:
    ...
    if any(token in text for token in ("驾驶舱", "业务域", "概况", "KPI", "运营")):
        return (
            "【校级驾驶舱场景补充】\n"
            "主题应体现多业务域运营总览，使用多 KPI 卡片、小型趋势图和状态标签，但保持浅色 SaaS BI，而不是大屏。"
        )
    return ""
